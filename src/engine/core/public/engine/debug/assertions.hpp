...
#define ZE_ASSERTF(condition, msg, ...) if(!(condition)) { logger::fatal("Assertion failed: {} (File: {}, Line: {})", fmt::format(msg, __VA_ARGS__), __FILE__, __LINE__); ZE_DEBUGBREAK(); }
#else 
#define ZE_ASSERT(condition) if(!(condition)) { logger::fatal("Assertion failed: {}", #condition); }
#define ZE_ASSERTF(condition, msg, ...) if(!(condition)) { logger::fatal("Assertion failed: {} ({})", fmt::format(msg, __VA_ARGS__), #condition); }
#endif
